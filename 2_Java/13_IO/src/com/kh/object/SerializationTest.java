...
//		s.objectsSave();
	...
	public void fileSave() {
		...
		try(FileOutputStream fos = new FileOutputStream(fileName);
			...
			System.out.println("person.obj 파일 정보 저장!");
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	
	// 역직렬화(deserialization) : 지정된 내용이나 전송받은 내용을 다시 복원하는 것
	public void fileRead() {
		// 1. 스트림 생성
		// FileInputStream : 파일 출력에 사용되는 기본 바이트 스트림 클래스
		// ObjectInputStream : 객체 단위로 출력 시에 사용되는 보조 바이트 스트림 클래스
		try(ObjectInputStream ois = new ObjectInputStream(new FileInputStream(fileName))) {
			// 2. 보조 스트림에 담겨진 객체 가져와서, 형변환
	...
	public void objectsRead() {
		...
		try(ObjectInputStream ois = new ObjectInputStream(new FileInputStream(fileName))) {
			// 2. 보조 스트림에 담겨진 객체 가져와서, 형변환
//			while(true) {}
			for(Person pp : (List<Person>) ois.readObject())
