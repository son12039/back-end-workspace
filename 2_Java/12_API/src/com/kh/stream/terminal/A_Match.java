...
public class A_Match {
	...
	public void method2() {
		...
		boolean result = students.stream().filter(student -> student.getAge()>=20)
		.allMatch(student -> student.getGender().startsWith("남"));
		
		System.out.println(result);
		System.out.println();
		
		// 남학생들 중에 평균이 70점 이상인 학생이 한 명이라도 존재하는 지확인
		result = students.stream().filter(student -> student.getGender().startsWith("남"))
