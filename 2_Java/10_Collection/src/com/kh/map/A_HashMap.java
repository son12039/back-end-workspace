...
public class A_HashMap {
	...
	public void mehthod2() {
		...
		map.put("고래밥", new Snack("앙념치킨", 173));
		...
		// 2. 후렌치파이의 맛 정보 출력
		System.out.println(map.get("후렌치파이"));
		Set<Entry<String, Snack>> mapset = map.entrySet();	

			System.out.println(map.get("후렌치파이").getFlavor());
		
		// 3. 모든 과자의 칼로리 촙합과 평균 출력
		int sum = 0;
		for(Entry<String, Snack> m : mapset) {
			sum +=m.getValue().getCalorie();
