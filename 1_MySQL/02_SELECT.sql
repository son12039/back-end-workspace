/*   CRUD Create Read Update Delete 데이터편집
	...
	- SQL 종류
	  ...
	  4. DDL(Data Definition Language) : 데이터 정의어
        ...
        (CREATE : 생성, ALTER : 수정, DROP : 삭제) - "테이블 변경"과 관련
	  5. TCL(Transaction Constrol Language) : 트랜잭션을 제어하는 언어
		(COMMOT : 실행, ROLLBACK : 취소)
...
/*
    ...
    - 모든 컬럼을 조회할 경우 컬럼명 대신 * 기호 사용 (전체선택)
...
-- employee 테이블의 전체 사원들의 사번(emp_id), 이름(emp_name), 급여(salary)만을 조회alter
...
/*
    ...
    - 산술연산을 하면 컬렴명이 지저분해진다.
	  이때 컬럼명에 별칭을 부여해서 깔끔하게 보여줄 수 있다.
	- 별칭을 부여할 때 "띄어쓰기" 혹은 "특수문자"가 포함될 경우 반드시 큰따옴표(")로 감싸준다.	
*/
SELECT 
	emp_name "직원명",
    salary as "직원 월급",
...
-- emloyee 테이블에 부서코드(dept_code) 조회
...
FROM employee
WHERE dept_code = 'D1';
-- 2. 부서코드가 'D1'이 아닌 시원들의 사번(emp_id), 사원명(emp_name), 부서코드 조회
...
-- 4. 재직중(ent_yn 컬럼값이 'N')인 사원들의 사번 사원명 입사일(hire.date)조회
SELECT emp_name, hire_date, emp_id
FROM employee
WHERE ent_yn = 'N';
...
/*
    ...
    비교대상컬럼 LIKE '%문자%' => 비교대상컬럼값이 문자가 "포함"되는 것을 조회 (키워드 검색)
...
*/
-- 사원들 중 성이 전씨인 사원들의 emp_name/사원명, salary/급여, hire_date입사일 조회
...
SELECT emp_name,phone,email,emp_id
FROM employee
WHERE phone LIKE '__1%';

-- 이메일 중 _ 앞글자가 3글자인 사원들의 사번, 아룸, 이메일 조회
-- ESCAPE OPTION : 나만의 와일드 카드를 만들어서 사용!
SELECT email,emp_id,emp_name
FROM employee
-- WHERE email LIKE '___$_%' ESCAPE '$'; -- ESCAPE는 $를 _로 인식하게 만듦
WHERE email LIKE '___\_%';
-- 위의 사원들 이외의 사원들 조회
-- 논리부정연산자 : NOT // vsc의 ! 짝수번쓰면 그대로
SELECT email,emp_id,emp_name
FROM employee
-- WHERE NOT email LIKE '___$_%' ESCAPE '$';
WHERE NOT NOT NOT NOT NOT NOT email NOT LIKE '___$_%' ESCAPE '$';

/*
	IS NULL / IS NOT NULL
    - 컬럼값에 NULL이 있을 경우 NULL 값 비교에 사용되는 연산자
*/
-- 보너스를 받지 않는 사원(bonus 값이 null)들의 사번(emp_id),이름,급여,보너스(bonus)조회
...
WHERE dept_code IN ('D5', 'D6', 'D8');  -- 윗줄이랑 같음!  
...
FROM employee
-- ORDER BY  bonus ASC; = ORDER BY  bonus ||| -- 보너스 기준으로 오름차순 정렬 (null이 먼저 정렬)
ORDER BY 2 DESC; -- 내림차순 정렬 
...
-- LIMIT 절은 두 개의 값이 있을 수 있음!(n,m) index n번째부터 m번째까지
...
-- 2. 사수가 없고 부서배치도 받지 않은 사원들의 사원명 name
-- 	  사수사번(manager_id), 부서코드(dept_code) 조회
SELECT emp_name
FROM employee
WHERE manager_id IS NULL AND dept_code IS NULL;
-- 3. 연봉(보너스x)이 4000만원 이상이고 보너스를 받지 않은 사원들의 사번e id,사원명,굽요,보너스
SELECT emp_id, emp_name,salary,bonus
FROM employee
...
-- 보너스 포함연봉 : (salary + salary*bonus) *12
