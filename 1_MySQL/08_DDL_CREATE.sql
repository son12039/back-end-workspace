...
MySQL에서 객체(구조) : 테이블(Table), 뷰(Vieㅈ), 인덱스(Index),
...
/*
    ...
    - 해당 컬럼에 반드시 값이 있어야만 하는 경우
		(즉, 해당 컬럼에 절대 NULL이 들어와서는 안 되는 경우)
	- 추가/수정시 NULL 값을 허용하지 않도록 제한
*/
CREATE TABLE mem_notnull(
	mem_no INT NOT NULL,
    mem_id VARCHAR(20) NOT NULL,
    mem_pwe VARCHAR(20) NOT NULL,
...
CREATE TABLE mem_check(
    ...
    CONSTRAINT mem_gender_check_not_null CHECK (gender IS NOT NULL)
);
...
-- >> 회원번호(mem_no)가 동일해도 insert가 되는 게 문제!
...
/*
    ...
    - RPIMARY KEY 제약조건을 부여하면 그 컬럼에 자동으로 NOT NULL + UNIQUE 조건이 설정
    - 한 테이블 당 오로지 한 개만 설정
    --> 간혹 복합키 설정도 가능
*/
...
WHERE grade_code = 2;
-- >> 자식 테이블(member)에 2라는 값을 사용하고 있기 떄문에 삭제 X

-- mem_grade 테이블에서 grade_code가 2인 등급 삭제
DELETE FROM mem_grade
WHERE grade_code = 3;
-- >> 자식테이블(member)에 3이라는 값을 사용하지 않기 때문에 삭제 O
SELECT * FROM member;
...
-- employee 테이블에서 사번, 직원명, 급여 , 연봉만 저장한employee 테이블로 생성
CREATE TABLE employee_copy
