...
/*
    ...
    BEGIN
		DECLATE 반환값 자료형
        결과에 해당하는 SQL문 등 계산식 (resulf로 처리)
        RETURN 반환값
    END //
    
    DELIMITER ;
*/
-- 사번을 입력받아 해당 사원의 연봉의 계산하고 리턴하는 함수 생성
DELIMITER //
...
-- 방금만든 salary_calc 함수를 이용해서 연봉이 4000만원 이상인 
-- 사원의 사번 사원명 급여 연봉 조회
...
-- employee 테이블에서 UPDATE 수행후 업데이티ㅡ 실행 메세지를 LOG에 담는
-- trg_02 트리거 생성

-- ODL : 수정, 삭제 전 데이터에 접근 가능
...
-- employee 테이블에서 dept_code가 D6인 직원들의 부서코드 D3으로 변경
...
CREATE TABLE prodetail(
    ...
    amount INT NOT NULL,
    status CHAR(2),
    constraint statuscheck CHECK (status IN ('입고', '출고')),
...
/*
    ...
    product 테이블에 매번 자동으로 재고수량 UPDATE 되게끔 트리거 trg_03 정의
    IF 조건 
    THEN SQL문
    ELSE SQL문
    END IF;
